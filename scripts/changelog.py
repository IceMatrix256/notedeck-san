...
Entry = namedtuple("Entry", ["commit", "pullreq", "content", "section", "author", "crates"])
...
def get_commit_crates(commit):
    """Get the set of crate names touched by a commit."""
    files = git("diff-tree --no-commit-id --name-only -r {commit}".format(commit=commit))
    crates = set()
...
def get_log_entries(commitrange):
    ...
    for l in logs.split('\n'):
        ...
        crates = get_commit_crates(commit)
        e = Entry(commit, pullreq, m.group(2), m.group(1).lower(), author, crates)
        entries.append(e)
